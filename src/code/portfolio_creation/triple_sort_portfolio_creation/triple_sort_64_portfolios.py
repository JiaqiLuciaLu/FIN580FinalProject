...
`reference_code/1_Portfolio_Creation/Triple_Sort_Portfolio_Creation/TripleSort64_Portfolios.R`.

Build 4x4x4 = 64 triple-sorted portfolios for (LME, feat1, feat2), subtract
...
def triple_sort_helper(df_tmp, feat1, feat2, feat3):
    ...
    for mon in range(1, 13):
        ...
        df_m["1"] = ntile(df_m[feat1].values, 4)
...
def triple_sort(data_path, feat1, feat2, feat3, y_min, y_max):
    ret_table = np.zeros((64, (y_max - y_min + 1) * 12))
    y_time_stamp = 1
...
def gen_triple_sort_64(feats_list, feat1, feat2,
                       y_min=utils.Y_MIN, y_max=utils.Y_MAX,
                       data_chunk_path=utils.DATA_CHUNK_DIR,
                       output_path=utils.PY_TS64_PORT_DIR,
